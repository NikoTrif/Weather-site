...
async function LoadWeather(lat, lon) {
  ...
  currentTemp.textContent = UnitConvert(
    weatherData.main.temp,
    usedUnit.textContent
  );
  weather.textContent = weatherData.weather[0].main;
  feel.textContent = UnitConvert(
...
async function LoadForecast(lat, lon) {
  ...
  document.querySelectorAll("table .table-header tr .col").forEach((e) => {
    e.textContent = new Date(fcstArray[y]?.date).toLocaleString("en", {
      weekday: "long",
    });
    y++;
  });
  ...
  fcstArray.forEach((e) => {
    tableBodyRow[0].innerHTML =
      tableBodyRow[0].innerHTML +
      `<td><img src="http://openweathermap.org/img/wn/${e.icon}@2x.png"></td>`;
    tableBodyRow[1].innerHTML =
      tableBodyRow[1].innerHTML +
      `<td>Max:<br>${UnitConvert(e.max, usedUnit.textContent)}° ${
        usedUnit.textContent
      }</td>`;
    tableBodyRow[2].innerHTML =
      tableBodyRow[2].innerHTML +
      `<td>Min:<br>${UnitConvert(e.min, usedUnit.textContent)}° ${
        usedUnit.textContent
      }</td>`;
  });
}
...
searcher.addEventListener("keyup", function () {
  let searcherVal = searcher.value
    .replaceAll(",", "$")
    .replaceAll("$ ", "$")
    .split("$");
  ...
  city = [
    ...new Set([
      ...
      ...cities.filter((eb) => {
        return eb.admin_name
          ?.toLowerCase()
          ?.includes(searcherVal[0].toLowerCase());
      }),
      ...cities.filter((ec) => {
        return ec.city_ascii
          ?.toLowerCase()
          ?.includes(searcherVal[0].toLowerCase());
      }),
    ]),
  ...
  if (city.length > 0 && city.length < 10) {
    ...
    city.forEach((eb) => {
      selectCity.innerHTML =
        selectCity.innerHTML +
        `<option value = "${eb.lat},${eb.lng}"> ${eb.city}, ${eb.country}</option>`;
    });
